import { useCallback, useEffect, useRef, useState, type ReactNode } from 'react';
import {
  Animated,
  Easing,
  Pressable,
  ScrollView,
  StyleSheet,
  Text,
  View,
  useWindowDimensions,
} from 'react-native';
import { SafeAreaView, useSafeAreaInsets } from 'react-native-safe-area-context';
import Svg, { Circle, Defs, G, Line, LinearGradient, Path, Polygon, Rect, Stop, Text as SvgText } from 'react-native-svg';

import { BottomNavigation } from '@/components/bottom-navigation';
import { MonikeHeader } from '@/components/monike-header';
import { BottomTabInset, CardRadius, Fonts, MonikeColors, ScreenPadding } from '@/constants/theme';
import { apiFetch } from '@/services/api';
import { useSWR } from '@/hooks/use-swr';

// ─── Backend types (mirror Pydantic models exactly) ──────────────────────────

type DowBar = {
  dow: number;           // 0=Mon … 6=Sun  (backend maps postgres DOW)
  day_name: string;
  avg_spend: number;
  total_spend: number;
  days_recorded: number;
};

type MonthlyPoint = {
  month: number;
  year: number;
  month_label: string;
  total_spend: number;
  pct_change: number;   // 0 for first entry
};

type HeatmapCell = {
  hour: number;          // 0-23
  dow: number;           // 0=Mon … 6=Sun
  transaction_count: number;
};

type PatternsResponse = {
  dow_bars: DowBar[];
  monthly_points: MonthlyPoint[];
  heatmap: HeatmapCell[];
  weekend_avg: number;
  weekday_avg: number;
};

// ─── Validation helpers ──────────────────────────────────────────────────────

function validatePatternsResponse(data: PatternsResponse): string[] {
  const warnings: string[] = [];

  if (data.dow_bars.length === 0) {
    warnings.push('dow_bars is empty — no daily-log data found for day-of-week chart.');
  } else if (data.dow_bars.length < 7) {
    warnings.push(
      `dow_bars has only ${data.dow_bars.length}/7 days — some days have no recorded spend.`,
    );
  }

  for (const bar of data.dow_bars) {
    if (bar.dow < 0 || bar.dow > 6) {
      warnings.push(`dow_bars: unexpected dow value ${bar.dow} (expected 0–6).`);
    }
    if (bar.avg_spend < 0) {
      warnings.push(`dow_bars: negative avg_spend (${bar.avg_spend}) for dow=${bar.dow}.`);
    }
  }

  if (data.monthly_points.length === 0) {
    warnings.push('monthly_points is empty — no data to render monthly trend chart.');
  }

  if (data.heatmap.length === 0) {
    warnings.push('heatmap is empty — no transactions table rows found with debit > 0.');
  }

  for (const cell of data.heatmap) {
    if (cell.hour < 0 || cell.hour > 23) {
      warnings.push(`heatmap: invalid hour=${cell.hour} (expected 0–23).`);
    }
    if (cell.dow < 0 || cell.dow > 6) {
      warnings.push(`heatmap: invalid dow=${cell.dow} (expected 0–6).`);
    }
  }

  if (data.weekend_avg < 0) warnings.push(`weekend_avg is negative (${data.weekend_avg}).`);
  if (data.weekday_avg < 0) warnings.push(`weekday_avg is negative (${data.weekday_avg}).`);

  return warnings;
}

// ─── Constants ────────────────────────────────────────────────────────────────

const DOW_KEYS = ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'];
const HOUR_LABELS = ['12am', '3am', '6am', '9am', '12pm', '3pm', '6pm', '9pm'];

type PatternTab = 'DAY OF WEEK' | 'MONTHLY' | 'TIME OF DAY';
const tabs: PatternTab[] = ['DAY OF WEEK', 'MONTHLY', 'TIME OF DAY'];

// ─── Formatters ───────────────────────────────────────────────────────────────

function formatNaira(value: number) {
  return new Intl.NumberFormat('en-NG', { maximumFractionDigits: 0 }).format(Math.abs(value));
}

function formatHour(hour: number) {
  if (hour === 0) return '12am';
  if (hour < 12) return `${hour}am`;
  if (hour === 12) return '12pm';
  return `${hour - 12}pm`;
}

// ─── Skeleton primitives ──────────────────────────────────────────────────────

function SkeletonBox({ width, height, style }: { width?: number | string; height: number; style?: object }) {
  const pulse = useRef(new Animated.Value(0.4)).current;

  useEffect(() => {
    Animated.loop(
      Animated.sequence([
        Animated.timing(pulse, { toValue: 1, duration: 700, easing: Easing.inOut(Easing.ease), useNativeDriver: true }),
        Animated.timing(pulse, { toValue: 0.4, duration: 700, easing: Easing.inOut(Easing.ease), useNativeDriver: true }),
      ]),
    ).start();
  }, [pulse]);

  return (
    <Animated.View
      style={[
        { width: width ?? '100%', height, borderRadius: 8, backgroundColor: MonikeColors.bgElevated, opacity: pulse },
        style,
      ]}
    />
  );
}

function SkeletonCard() {
  return (
    <View style={[styles.sectionCard, { gap: 10 }]}>
      <SkeletonBox height={140} />
      <View style={{ flexDirection: 'row', gap: 8 }}>
        <SkeletonBox width="30%" height={14} />
        <SkeletonBox width="25%" height={14} />
        <SkeletonBox width="20%" height={14} />
      </View>
    </View>
  );
}

function SkeletonInsightCard() {
  return (
    <View style={styles.insightCard}>
      <SkeletonBox width="40%" height={13} />
      <View style={{ marginTop: 8, gap: 5 }}>
        <SkeletonBox height={13} />
        <SkeletonBox width="80%" height={13} />
      </View>
    </View>
  );
}

function SkeletonStatPair() {
  return (
    <View style={styles.statPairRow}>
      <View style={[styles.rhythmStatCard, { gap: 8 }]}>
        <SkeletonBox width="50%" height={10} />
        <SkeletonBox width="70%" height={20} />
        <SkeletonBox width="30%" height={10} />
      </View>
      <View style={[styles.rhythmStatCard, { gap: 8 }]}>
        <SkeletonBox width="50%" height={10} />
        <SkeletonBox width="70%" height={20} />
        <SkeletonBox width="30%" height={10} />
      </View>
    </View>
  );
}

function TabSkeletons() {
  return (
    <View style={{ gap: 16 }}>
      <SkeletonCard />
      <SkeletonInsightCard />
      <SkeletonStatPair />
    </View>
  );
}

// ─── Backend warning banner ───────────────────────────────────────────────────

function BackendWarnings({ warnings }: { warnings: string[] }) {
  if (warnings.length === 0) return null;
  return (
    <View style={styles.warningBanner}>
      <Text style={styles.warningTitle}>⚠ Backend data issues</Text>
      {warnings.map((w, i) => (
        <Text key={i} style={styles.warningText}>• {w}</Text>
      ))}
    </View>
  );
}

// ─── Shared small components ──────────────────────────────────────────────────

function SectionCard({ children }: { children: ReactNode }) {
  return <View style={styles.sectionCard}>{children}</View>;
}

function InsightCard({ title, text }: { title: string; text: string }) {
  return (
    <View style={styles.insightCard}>
      <Text style={styles.insightTitle}>{title}</Text>
      <Text style={styles.insightText}>{text}</Text>
    </View>
  );
}

function TabSelector({ activeTab, onChange }: { activeTab: PatternTab; onChange: (tab: PatternTab) => void }) {
  return (
    <View style={styles.tabSelector}>
      {tabs.map((tab) => {
        const active = tab === activeTab;
        return (
          <Pressable key={tab} style={styles.tabButton} onPress={() => onChange(tab)}>
            <Text style={[styles.tabLabel, active && styles.tabLabelActive]}>{tab}</Text>
            <View style={[styles.tabUnderline, active && styles.tabUnderlineActive]} />
          </Pressable>
        );
      })}
    </View>
  );
}

// ─── Day of Week tab ─────────────────────────────────────────────────────────

type SelectedDay = DowBar & { key: string };

function DayTooltip({ day }: { day: SelectedDay }) {
  return (
    <View style={styles.tooltipCard}>
      <Text style={styles.tooltipTitle}>{day.day_name}</Text>
      <Text style={styles.tooltipValue}>Avg ₦{formatNaira(day.avg_spend)}/day</Text>
      <Text style={styles.tooltipSub}>
        Total ₦{formatNaira(day.total_spend)} ({day.days_recorded} days recorded)
      </Text>
    </View>
  );
}

function RhythmStat({
  badge, badgeTone, label, sub, value,
}: { badge?: string; badgeTone?: string; label: string; sub: string; value: number }) {
  return (
    <View style={styles.rhythmStatCard}>
      <View style={styles.statHeaderRow}>
        <Text style={styles.rhythmLabel}>{label}</Text>
        {badge ? (
          <Text style={[styles.statBadge, { color: badgeTone, borderColor: badgeTone }]}>{badge}</Text>
        ) : null}
      </View>
      <Text style={styles.rhythmValue}>₦{formatNaira(value)}</Text>
      <Text style={styles.rhythmSub}>{sub}</Text>
    </View>
  );
}

function DayOfWeekTab({ dowBars, weekdayAvg, weekendAvg }: {
  dowBars: DowBar[];
  weekdayAvg: number;
  weekendAvg: number;
}) {
  const [selectedDay, setSelectedDay] = useState<SelectedDay | null>(null);
  const anim = useRef(new Animated.Value(0)).current;

  // Build a full 7-slot array keyed by dow (0=Mon…6=Sun), filling gaps with null
  const slots: (DowBar | null)[] = Array.from({ length: 7 }, (_, i) =>
    dowBars.find((b) => b.dow === i) ?? null,
  );

  const maxAvg = Math.max(...dowBars.map((b) => b.avg_spend), 1);
  const maxBar = dowBars.reduce<DowBar | null>((best, b) => (!best || b.avg_spend > best.avg_spend ? b : best), null);
  const minBar = dowBars.reduce<DowBar | null>((best, b) => (!best || b.avg_spend < best.avg_spend ? b : best), null);

  useEffect(() => {
    anim.setValue(0);
    Animated.timing(anim, { toValue: 1, duration: 480, easing: Easing.out(Easing.cubic), useNativeDriver: false }).start();
  }, [anim]);

  return (
    <Animated.View style={styles.tabContent}>
      <SectionCard>
        <View style={styles.chartGridLayer}>
          {[0, 1, 2, 3].map((line) => (
            <View key={line} style={[styles.gridLine, { bottom: 28 + line * 38 }]}>
              <Text style={styles.gridLabel}>₦{formatNaira((maxAvg / 3) * line)}</Text>
              <View style={styles.dashedLine} />
            </View>
          ))}
        </View>
        <View style={styles.barChartRow}>
          {slots.map((bar, index) => {
            const key = DOW_KEYS[index];
            const isHigh = bar && maxBar && bar.dow === maxBar.dow;
            const isLow = bar && minBar && bar.dow === minBar.dow;
            const color = isHigh
              ? MonikeColors.signalRed
              : isLow
              ? MonikeColors.accentNeon
              : MonikeColors.accentPulse;

            return (
              <Pressable
                key={key}
                style={styles.dayBarWrap}
                onPress={() => bar ? setSelectedDay({ ...bar, key }) : null}
              >
                {bar ? (
                  <Text style={styles.barValue}>₦{formatNaira(bar.avg_spend)}</Text>
                ) : (
                  <Text style={styles.barValue}>—</Text>
                )}
                <View style={styles.barSlot}>
                  {isHigh ? <Text style={styles.crownMarker}>👑</Text> : null}
                  {bar ? (
                    <Animated.View
                      style={[
                        styles.dayBar,
                        {
                          backgroundColor: color,
                          height: anim.interpolate({
                            inputRange: [0, 1],
                            outputRange: [0, (bar.avg_spend / maxAvg) * 118],
                          }),
                        },
                      ]}
                    />
                  ) : (
                    <View style={[styles.dayBar, { height: 4, backgroundColor: MonikeColors.inkGhost }]} />
                  )}
                </View>
                <Text style={styles.axisLabel}>{key}</Text>
              </Pressable>
            );
          })}
        </View>
        {selectedDay ? <DayTooltip day={selectedDay} /> : null}
      </SectionCard>

      {maxBar && minBar ? (
        <InsightCard
          title="📊 Pattern Insight"
          text={`${maxBar.day_name}s are your most expensive day — you spend an average of ₦${formatNaira(maxBar.avg_spend)} — ${(maxBar.avg_spend / Math.max(minBar.avg_spend, 1)).toFixed(1)}× your cheapest day (${minBar.day_name} at ₦${formatNaira(minBar.avg_spend)}). Watch out.`}
        />
      ) : null}

      <View style={styles.statPairRow}>
        <RhythmStat label="WEEKDAYS" value={weekdayAvg} sub="Mon–Fri" />
        <RhythmStat
          label="WEEKENDS"
          value={weekendAvg}
          sub="Sat–Sun"
          badge={weekendAvg > weekdayAvg ? 'CAUTION ⚠' : 'SAVED ✓'}
          badgeTone={weekendAvg > weekdayAvg ? MonikeColors.signalAmber : MonikeColors.accentPulse}
        />
      </View>
    </Animated.View>
  );
}

// ─── Monthly tab ──────────────────────────────────────────────────────────────

function smoothPath(points: { x: number; y: number }[]) {
  return points.reduce((path, point, index) => {
    if (index === 0) return `M ${point.x} ${point.y}`;
    const prev = points[index - 1];
    const midX = (prev.x + point.x) / 2;
    return `${path} Q ${prev.x} ${prev.y} ${midX} ${(prev.y + point.y) / 2} T ${point.x} ${point.y}`;
  }, '');
}

type Status = 'HIGH' | 'MEDIUM' | 'LOW';

function StatusBadge({ status }: { status: Status }) {
  const palette = {
    HIGH: MonikeColors.signalRed,
    MEDIUM: MonikeColors.signalAmber,
    LOW: MonikeColors.accentPulse,
  }[status];
  return (
    <Text style={[styles.statusBadge, { color: palette, borderColor: palette }]}>{status}</Text>
  );
}

function MonthlyTab({ monthlyPoints }: { monthlyPoints: MonthlyPoint[] }) {
  const [selectedPoint, setSelectedPoint] = useState<MonthlyPoint | null>(null);

  if (monthlyPoints.length === 0) {
    return (
      <View style={styles.tabContent}>
        <View style={[styles.sectionCard, { alignItems: 'center', paddingVertical: 40 }]}>
          <Text style={styles.emptyText}>No monthly data yet</Text>
        </View>
      </View>
    );
  }

  const totals = monthlyPoints.map((p) => p.total_spend);
  const max = Math.max(...totals);
  const min = Math.min(...totals);
  const mean = totals.reduce((a, b) => a + b, 0) / totals.length;

  const step = Math.max(52, 280 / Math.max(monthlyPoints.length, 1));
  const points = monthlyPoints.map((mp, index) => ({
    ...mp,
    x: 38 + index * step,
    y: 154 - ((mp.total_spend - min) / Math.max(max - min, 1)) * 116,
  }));

  const viewWidth = 38 + (monthlyPoints.length - 1) * step + 40;
  const path = smoothPath(points.map(({ x, y }) => ({ x, y })));
  const area = `38,164 ${points.map(({ x, y }) => `${x},${y}`).join(' ')} ${points.at(-1)?.x ?? 298},164`;

  return (
    <View style={styles.tabContent}>
      <SectionCard>
        <ScrollView horizontal showsHorizontalScrollIndicator={false}>
          <Svg width={viewWidth} height={190} viewBox={`0 0 ${viewWidth} 190`}>
            <Defs>
              <LinearGradient id="monthlyArea" x1="0" y1="0" x2="0" y2="1">
                <Stop offset="0" stopColor={MonikeColors.accentPulse} stopOpacity="0.2" />
                <Stop offset="1" stopColor={MonikeColors.accentPulse} stopOpacity="0" />
              </LinearGradient>
            </Defs>
            {[0, 1, 2, 3].map((line) => {
              const y = 38 + line * 38;
              return (
                <Line
                  key={line}
                  x1={36} x2={viewWidth - 10} y1={y} y2={y}
                  stroke={MonikeColors.inkGhost}
                  strokeDasharray="4 6"
                  strokeWidth={1}
                />
              );
            })}
            {[0, 1, 2, 3].map((line) => (
              <SvgText key={line} x={0} y={40 + line * 38} fill={MonikeColors.inkMuted} fontSize={9} fontFamily={Fonts.mono}>
                ₦{formatNaira(max - ((max - min) / 3) * line)}
              </SvgText>
            ))}
            <Polygon points={area} fill="url(#monthlyArea)" />
            <Path d={path} fill="none" stroke={MonikeColors.accentPulse} strokeWidth={2.5} strokeLinecap="round" strokeLinejoin="round" />
            {points.map((point) => {
              const hot = point.total_spend > mean * 1.12;
              return (
                <G key={`${point.year}-${point.month}`} onPress={() => setSelectedPoint(point)}>
                  {hot ? (
                    <SvgText x={point.x - 3} y={point.y - 14} fill={MonikeColors.signalRed} fontSize={11} fontWeight="700">!</SvgText>
                  ) : null}
                  <Circle
                    cx={point.x} cy={point.y} r={7}
                    fill={hot ? MonikeColors.signalRed : MonikeColors.accentPulse}
                    stroke={MonikeColors.bgVoid}
                    strokeWidth={2}
                  />
                  <SvgText x={point.x - 10} y={180} fill={MonikeColors.inkMuted} fontSize={10} fontFamily={Fonts.mono}>
                    {point.month_label}
                  </SvgText>
                  <Rect x={point.x - 18} y={point.y - 18} width={36} height={36} fill="transparent" />
                </G>
              );
            })}
          </Svg>
        </ScrollView>
        {selectedPoint ? (
          <View style={styles.tooltipCard}>
            <Text style={styles.tooltipTitle}>{selectedPoint.month_label} {selectedPoint.year}</Text>
            <Text style={styles.tooltipValue}>₦{formatNaira(selectedPoint.total_spend)}</Text>
            <Text style={styles.tooltipSub}>
              {selectedPoint.pct_change === 0
                ? 'Baseline month'
                : `${selectedPoint.pct_change > 0 ? '▲' : '▼'} ${Math.abs(selectedPoint.pct_change).toFixed(1)}% vs previous`}
            </Text>
          </View>
        ) : null}
      </SectionCard>

      {/* Monthly table */}
      <View style={styles.monthlyTable}>
        {monthlyPoints.map((mp, index) => {
          const status: Status = mp.total_spend > mean * 1.08 ? 'HIGH' : mp.total_spend < mean * 0.92 ? 'LOW' : 'MEDIUM';
          return (
            <View key={`${mp.year}-${mp.month}`} style={[styles.monthRow, index % 2 === 1 && styles.monthRowStripe]}>
              <Text style={styles.monthCell}>{mp.month_label} {mp.year}</Text>
              <Text style={styles.monthTotal}>₦{formatNaira(mp.total_spend)}</Text>
              <Text style={[styles.monthChange, { color: mp.pct_change > 0 ? MonikeColors.signalRed : MonikeColors.accentPulse }]}>
                {index === 0 ? '—' : `${mp.pct_change > 0 ? '▲' : '▼'} ${Math.abs(mp.pct_change).toFixed(1)}%`}
              </Text>
              <StatusBadge status={status} />
            </View>
          );
        })}
        <View style={styles.averageRow}>
          <Text style={[styles.monthCell, styles.averageText]}>AVERAGE</Text>
          <Text style={[styles.monthTotal, styles.averageText]}>₦{formatNaira(mean)}</Text>
          <Text style={styles.monthChange}>—</Text>
          <Text style={styles.averageText}>MEAN</Text>
        </View>
      </View>
    </View>
  );
}

// ─── Time of Day tab ─────────────────────────────────────────────────────────

function heatColor(count: number) {
  if (count === 0) return MonikeColors.bgOverlay;
  if (count <= 2) return 'rgba(0,230,118,0.15)';
  if (count <= 5) return 'rgba(0,230,118,0.40)';
  if (count <= 9) return 'rgba(0,230,118,0.70)';
  return MonikeColors.accentPulse;
}

function peakColor(total: number) {
  if (total >= 34) return MonikeColors.accentPulse;
  if (total >= 18) return 'rgba(0,230,118,0.4)';
  return MonikeColors.bgElevated;
}

function HeatmapRow({
  cellWidth, dowIndex, heatmap, onSelect, rowIndex,
}: {
  cellWidth: number;
  dowIndex: number;
  heatmap: HeatmapCell[];
  onSelect: (cell: HeatmapCell) => void;
  rowIndex: number;
}) {
  const opacity = useRef(new Animated.Value(0)).current;
  useEffect(() => {
    opacity.setValue(0);
    Animated.timing(opacity, { toValue: 1, delay: rowIndex * 40, duration: 220, easing: Easing.out(Easing.quad), useNativeDriver: true }).start();
  }, [opacity, rowIndex]);

  return (
    <Animated.View style={[styles.heatRow, { opacity }]}>
      <Text style={styles.heatDayLabel}>{DOW_KEYS[dowIndex]}</Text>
      {Array.from({ length: 24 }, (_, hour) => {
        const cell = heatmap.find((c) => c.dow === dowIndex && c.hour === hour);
        const count = cell?.transaction_count ?? 0;
        return (
          <Pressable
            key={hour}
            onPress={() => cell ? onSelect(cell) : null}
            style={[styles.heatCell, { width: cellWidth, backgroundColor: heatColor(count) }]}
          />
        );
      })}
    </Animated.View>
  );
}

function TimeOfDayTab({ heatmap }: { heatmap: HeatmapCell[] }) {
  const { width } = useWindowDimensions();
  const [selectedCell, setSelectedCell] = useState<HeatmapCell | null>(null);
  const cellWidth = Math.max(8, (width - 60 - ScreenPadding * 2) / 24);

  const peakCell = heatmap.length > 0
    ? heatmap.reduce((best, c) => (c.transaction_count > best.transaction_count ? c : best), heatmap[0])
    : null;

  const hourlyTotals = Array.from({ length: 24 }, (_, hour) =>
    heatmap.filter((c) => c.hour === hour).reduce((sum, c) => sum + c.transaction_count, 0),
  );
  const peakHourTotal = Math.max(...hourlyTotals);
  const peakHour = hourlyTotals.indexOf(peakHourTotal);

  return (
    <View style={styles.tabContent}>
      <SectionCard>
        <View style={styles.hourLabelsRow}>
          <View style={styles.dayAxisSpacer} />
          {HOUR_LABELS.map((label, index) => (
            <Text key={label} style={[styles.hourLabel, { left: index * cellWidth * 3 }]}>{label}</Text>
          ))}
        </View>
        {Array.from({ length: 7 }, (_, dowIndex) => (
          <HeatmapRow
            key={dowIndex}
            cellWidth={cellWidth}
            dowIndex={dowIndex}
            heatmap={heatmap}
            onSelect={setSelectedCell}
            rowIndex={dowIndex}
          />
        ))}
        {selectedCell ? (
          <View style={styles.tooltipCard}>
            <Text style={styles.tooltipTitle}>{DOW_KEYS[selectedCell.dow]} {formatHour(selectedCell.hour)}</Text>
            <Text style={styles.tooltipValue}>{selectedCell.transaction_count} transactions</Text>
          </View>
        ) : null}
      </SectionCard>

      {peakCell ? (
        <InsightCard
          title="🫀 Pattern Insight"
          text={`You transact most around ${formatHour(peakCell.hour)} on ${DOW_KEYS[peakCell.dow]}s (${peakCell.transaction_count} txns). Your quietest window is 1–5 AM.`}
        />
      ) : null}

      <View style={styles.peakStripCard}>
        <Text style={[styles.peakLabel, { left: `${(peakHour / 24) * 100}%` }]}>
          peak: {formatHour(peakHour).toLowerCase()}
        </Text>
        <View style={styles.peakTimeline}>
          {hourlyTotals.map((total, hour) => (
            <View key={hour} style={[styles.peakSegment, { backgroundColor: peakColor(total) }]} />
          ))}
        </View>
      </View>
    </View>
  );
}

// ─── Error state ──────────────────────────────────────────────────────────────

function ErrorState({ error, onRetry }: { error: Error; onRetry: () => void }) {
  return (
    <View style={styles.errorCard}>
      <Text style={styles.errorTitle}>Failed to load patterns</Text>
      <Text style={styles.errorText}>{error.message}</Text>
      <Pressable style={styles.retryButton} onPress={onRetry}>
        <Text style={styles.retryText}>Retry</Text>
      </Pressable>
    </View>
  );
}

// ─── Root screen ─────────────────────────────────────────────────────────────

const patternsFetcher = (key: string) => apiFetch<PatternsResponse>(key);

export default function PatternsScreen() {
  const [activeTab, setActiveTab] = useState<PatternTab>('DAY OF WEEK');
  const insets = useSafeAreaInsets();
  const slide = useRef(new Animated.Value(0)).current;

  const { data, error, isLoading, mutate } = useSWR<PatternsResponse>(
    '/patterns',
    useCallback(patternsFetcher, []),
  );

  const warnings = data ? validatePatternsResponse(data) : [];

  const changeTab = (tab: PatternTab) => {
    if (tab === activeTab) return;
    Animated.timing(slide, { toValue: -18, duration: 100, easing: Easing.inOut(Easing.quad), useNativeDriver: true }).start(() => {
      setActiveTab(tab);
      slide.setValue(18);
      Animated.timing(slide, { toValue: 0, duration: 200, easing: Easing.inOut(Easing.quad), useNativeDriver: true }).start();
    });
  };

  return (
    <View style={styles.root}>
      <SafeAreaView style={styles.safeArea} edges={['top']}>
        <ScrollView
          showsVerticalScrollIndicator={false}
          contentContainerStyle={[styles.content, { paddingBottom: insets.bottom + BottomTabInset + 24 }]}
        >
          <MonikeHeader title="Patterns" />

          <View style={styles.titleBlock}>
            <Text style={styles.screenTitle}>PATTERNS</Text>
            <Text style={styles.screenSubtitle}>Your spending rhythms</Text>
          </View>

          <TabSelector activeTab={activeTab} onChange={changeTab} />

          <BackendWarnings warnings={warnings} />

          {error ? (
            <ErrorState error={error} onRetry={mutate} />
          ) : isLoading ? (
            <TabSkeletons />
          ) : data ? (
            <Animated.View style={{ transform: [{ translateX: slide }] }}>
              {activeTab === 'DAY OF WEEK' ? (
                <DayOfWeekTab
                  dowBars={data.dow_bars}
                  weekdayAvg={data.weekday_avg}
                  weekendAvg={data.weekend_avg}
                />
              ) : null}
              {activeTab === 'MONTHLY' ? (
                <MonthlyTab monthlyPoints={data.monthly_points} />
              ) : null}
              {activeTab === 'TIME OF DAY' ? (
                <TimeOfDayTab heatmap={data.heatmap} />
              ) : null}
            </Animated.View>
          ) : null}
        </ScrollView>
      </SafeAreaView>
      <BottomNavigation activeRoute="patterns" />
    </View>
  );
}

// ─── Styles ───────────────────────────────────────────────────────────────────

const styles = StyleSheet.create({
  root: { flex: 1, backgroundColor: MonikeColors.bgVoid },
  safeArea: { flex: 1 },
  content: { paddingHorizontal: ScreenPadding, gap: 18 },
  titleBlock: { alignItems: 'center', gap: 5 },
  screenTitle: { color: MonikeColors.inkPrimary, fontFamily: Fonts.heading, fontSize: 22, fontWeight: '700', letterSpacing: 0.8 },
  screenSubtitle: { color: MonikeColors.inkSecondary, fontFamily: Fonts.sans, fontSize: 12 },
  tabSelector: { flexDirection: 'row', borderBottomWidth: 1, borderBottomColor: MonikeColors.inkGhost },
  tabButton: { flex: 1, alignItems: 'center', paddingTop: 6 },
  tabLabel: { color: MonikeColors.inkMuted, fontFamily: Fonts.sans, fontSize: 13, fontWeight: '400' },
  tabLabelActive: { color: MonikeColors.inkPrimary, fontWeight: '600' },
  tabUnderline: { marginTop: 10, width: '100%', height: 2, backgroundColor: 'transparent' },
  tabUnderlineActive: { backgroundColor: MonikeColors.accentPulse },
  tabContent: { gap: 16 },
  sectionCard: { backgroundColor: MonikeColors.bgSurface, borderWidth: 1, borderColor: MonikeColors.inkGhost, borderRadius: CardRadius, padding: 14, overflow: 'hidden' },
  chartGridLayer: { position: 'absolute', left: 14, right: 14, top: 14, height: 160 },
  gridLine: { position: 'absolute', left: 0, right: 0, flexDirection: 'row', alignItems: 'center', gap: 8 },
  gridLabel: { width: 42, color: MonikeColors.inkMuted, fontFamily: Fonts.mono, fontSize: 9 },
  dashedLine: { flex: 1, borderTopWidth: 1, borderStyle: 'dashed', borderColor: MonikeColors.inkGhost },
  barChartRow: { height: 184, flexDirection: 'row', alignItems: 'flex-end', paddingLeft: 44, gap: 9 },
  dayBarWrap: { flex: 1, alignItems: 'center', gap: 5 },
  barValue: { color: MonikeColors.inkSecondary, fontFamily: Fonts.mono, fontSize: 10 },
  barSlot: { height: 128, justifyContent: 'flex-end', alignItems: 'center' },
  dayBar: { width: 22, borderTopLeftRadius: 6, borderTopRightRadius: 6 },
  crownMarker: { color: MonikeColors.signalRed, fontFamily: Fonts.mono, fontSize: 9, marginBottom: 3 },
  axisLabel: { color: MonikeColors.inkMuted, fontFamily: Fonts.mono, fontSize: 10 },
  tooltipCard: { marginTop: 12, backgroundColor: MonikeColors.bgOverlay, borderWidth: 1, borderColor: MonikeColors.accentPulse, borderRadius: 12, padding: 12 },
  tooltipTitle: { color: MonikeColors.inkPrimary, fontFamily: Fonts.heading, fontSize: 14, fontWeight: '700' },
  tooltipValue: { color: MonikeColors.accentPulse, fontFamily: Fonts.mono, fontSize: 14, fontWeight: '700', marginTop: 5 },
  tooltipSub: { color: MonikeColors.inkMuted, fontFamily: Fonts.mono, fontSize: 12, marginTop: 3 },
  insightCard: { backgroundColor: MonikeColors.bgSurface, borderWidth: 1, borderColor: MonikeColors.inkGhost, borderRadius: CardRadius, padding: 14 },
  insightTitle: { color: MonikeColors.accentPulse, fontFamily: Fonts.heading, fontSize: 13, fontWeight: '600' },
  insightText: { color: MonikeColors.inkSecondary, fontFamily: Fonts.sans, fontSize: 13, lineHeight: 21, marginTop: 8 },
  statPairRow: { flexDirection: 'row', gap: 8 },
  rhythmStatCard: { flex: 1, backgroundColor: MonikeColors.bgSurface, borderWidth: 1, borderColor: MonikeColors.inkGhost, borderRadius: CardRadius, padding: 13 },
  statHeaderRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', minHeight: 20 },
  rhythmLabel: { color: MonikeColors.inkMuted, fontFamily: Fonts.sans, fontSize: 10 },
  rhythmValue: { color: MonikeColors.inkPrimary, fontFamily: Fonts.mono, fontSize: 18, fontWeight: '700', marginTop: 8 },
  rhythmSub: { color: MonikeColors.inkMuted, fontFamily: Fonts.sans, fontSize: 10, marginTop: 3 },
  statBadge: { borderWidth: 1, borderRadius: 999, paddingHorizontal: 6, paddingVertical: 2, fontFamily: Fonts.mono, fontSize: 8, fontWeight: '700' },
  monthlyTable: { borderRadius: CardRadius, borderWidth: 1, borderColor: MonikeColors.inkGhost, overflow: 'hidden' },
  monthRow: { minHeight: 48, flexDirection: 'row', alignItems: 'center', backgroundColor: MonikeColors.bgSurface, paddingHorizontal: 12 },
  monthRowStripe: { backgroundColor: MonikeColors.bgStripe },
  monthCell: { flex: 1.1, color: MonikeColors.inkPrimary, fontFamily: Fonts.sans, fontSize: 13 },
  monthTotal: { flex: 1, color: MonikeColors.inkPrimary, fontFamily: Fonts.mono, fontSize: 13, fontWeight: '700' },
  monthChange: { flex: 0.9, color: MonikeColors.inkMuted, fontFamily: Fonts.mono, fontSize: 12 },
  statusBadge: { overflow: 'hidden', borderWidth: 1, borderRadius: 999, paddingHorizontal: 8, paddingVertical: 3, fontFamily: Fonts.mono, fontSize: 9, fontWeight: '700' },
  averageRow: { minHeight: 50, flexDirection: 'row', alignItems: 'center', backgroundColor: MonikeColors.bgElevated, paddingHorizontal: 12 },
  averageText: { fontWeight: '800' },
  hourLabelsRow: { height: 18, marginLeft: 34, position: 'relative' },
  dayAxisSpacer: { width: 34 },
  hourLabel: { position: 'absolute', color: MonikeColors.inkMuted, fontFamily: Fonts.mono, fontSize: 9 },
  heatRow: { flexDirection: 'row', alignItems: 'center', marginBottom: 4 },
  heatDayLabel: { width: 34, color: MonikeColors.inkMuted, fontFamily: Fonts.sans, fontSize: 10 },
  heatCell: { height: 28, borderRadius: 2, marginRight: 1 },
  peakStripCard: { backgroundColor: MonikeColors.bgSurface, borderWidth: 1, borderColor: MonikeColors.inkGhost, borderRadius: CardRadius, padding: 14, paddingTop: 28 },
  peakLabel: { position: 'absolute', top: 9, color: MonikeColors.inkSecondary, fontFamily: Fonts.mono, fontSize: 10 },
  peakTimeline: { flexDirection: 'row', height: 8, borderRadius: 4, overflow: 'hidden' },
  peakSegment: { flex: 1, height: 8, marginRight: 1 },
  emptyText: { color: MonikeColors.inkMuted, fontFamily: Fonts.mono, fontSize: 13 },
  errorCard: { backgroundColor: MonikeColors.bgSurface, borderWidth: 1, borderColor: MonikeColors.signalRed, borderRadius: CardRadius, padding: 18, gap: 10 },
  errorTitle: { color: MonikeColors.signalRed, fontFamily: Fonts.heading, fontSize: 15, fontWeight: '700' },
  errorText: { color: MonikeColors.inkSecondary, fontFamily: Fonts.mono, fontSize: 12 },
  retryButton: { alignSelf: 'flex-start', backgroundColor: MonikeColors.bgElevated, borderRadius: 8, paddingHorizontal: 16, paddingVertical: 8 },
  retryText: { color: MonikeColors.accentPulse, fontFamily: Fonts.mono, fontSize: 13, fontWeight: '700' },
  warningBanner: { backgroundColor: 'rgba(255,179,0,0.08)', borderWidth: 1, borderColor: MonikeColors.signalAmber, borderRadius: CardRadius, padding: 12, gap: 6 },
  warningTitle: { color: MonikeColors.signalAmber, fontFamily: Fonts.heading, fontSize: 13, fontWeight: '700' },
  warningText: { color: MonikeColors.inkSecondary, fontFamily: Fonts.mono, fontSize: 11 },
});